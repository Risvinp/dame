...
line = LINE("xemoolemal@gmail.com","keeplove11") #Input Your Email and Password Here
#=======================================================================================================================
myMid = line.profile.mid
programStart = time.time()
oepoll = OEPoll(line)
#emal = ["u7c12144cc680c22acfd886ed81908b75"]
admin = ["u2ef0fc5aa2a83069d8f8a6a09801ff93","u7c12144cc680c22acfd886ed81908b75"]
protectinvite = []
protectkick = []
protectjoin = []
protectqr = []
...
#================BATAS================
       ...
       if msg._from in admin:
        line.sendMessage(to, 'dame cantik here')
    elif cmd == "kickall":
        line.sendMessage(to, 'Sorry, u r not my bouussssss')
    elif cmd == "yuu":
       if msg._from in admin:
        line.sendMessage(to, 'ＧＩＲ ＴＯＴ ＧＩＲ ＴＯＴ')
        line.sendMessage(to, 'ɢɪʀ ᴛᴏᴛ ɢɪʀ ᴛᴏᴛ')
        line.sendMessage(to, 'ᴍɪɴɢɢɪʀ Ｎｇｅｎｔｏｏｄ')
        line.sendMessage(to, 'ᴏʀᴀɴɢ ɢᴀɴᴛᴇɴɢ ᴍᴀᴜ ʟᴇᴡᴀᴛ')
        line.sendMessage(to, 'ᴛᴡɪɴᴢ ɴɪ ʙᴏᴏᴏᴏᴜꜱꜱ')
        line.sendMessage(to, 'Ｓｕｐｐｏｒｔ Ｂｙ ：')
        line.sendContact(to, 'u7c12144cc680c22acfd886ed81908b75')
        sendContact(to, 'u101c67715b7ba1a31522ce91eff56a6c')
        sendContact(to, 'u101c67715b7ba1a31522ce91eff56a6c')
        sendMessage(to, 'Ｓｅｅ ｙｏｕ Ｎｇｅｎｔｏｏｄ♪')
    elif cmd == 'creator':
       if msg._from in admin:
        sendContact(to, 'u7c12144cc680c22acfd886ed81908b75')
        sendContact(to, 'u101c67715b7ba1a31522ce91eff56a6c')
        sendContact(to, 'u101c67715b7ba1a31522ce91eff56a6c')
    elif cmd == 'cekbot':
       if msg._from in admin:
       ...
       if msg._from in admin:
        text = "Gua pulang dulu ya, byee anak ngent "
        sendFooter(to,text)
        G = line.getGroup(to)
        line.leaveGroup(to)
...
#================Protection================
       ...
       if msg._from in admin:
                                ...
                                sendFooter(to,"• Protectlist •\n\n- QR Protection :\n"+ma+"\n- Lock Kick :\n"+mb+"\n- Lock Join :\n"+md+"\n- Deny Invitation :\n"+me+"\n- Lock Cancel :\n"+mf+"\nTotal「%s」Protect Group\n\n  Twinz Nih Bous  " % (str(len(settings["protectqr"])+len(settings["protectkick"])+len(settings["protectjoin"])+len(settings["protectinvite"])+len(settings["protectcancel"]))))
    ...
    try:
        ...
        if op.type == 19 or op.type == 133:
          if op.param3 in myMid:
                ban["blacklist"].append(op.param2)
                group = "u7c12144cc680c22acfd886ed81908b75" #Can Change for Mid Group or Line Personal Chat
                ...
                data = {
                                           ...
                                           "linkUrl": "https://line.me/ti/p/~gakmakeline_",
                                         }
                                     }
          ...
          if op.param3 in settings["whitelist"]:
                if op.param2 not in settings["whitelist"]:
                    try:
                        ...
                        sendFooter(op.param1,"ᴊᴀɴɢᴀɴ ᴋɪᴄᴋ ᴛᴇᴍᴇɴ ɢᴜᴀ ɴɢᴇɴᴛᴏᴏᴏᴏᴛ!!")
                    except:pass
          ...
          if op.param3 in settings["whitelist"]:
                if op.param2 not in settings["whitelist"]:
                    try:
                        ...
                        line.kickoutFromGroup(op.param1,[op.param2])
                        line.inviteIntoGroup(op.param1,[op.param3])
                        sendFooter(op.param1,"ɪᴛᴜ ᴛᴇᴍᴇɴ ɢᴜᴀ ᴊᴀɴɢᴀɴ ᴅɪ ᴄᴀɴᴄᴇʟ ᴋᴏɴᴛᴏʟ!!!")
                    except:pass
